...
import (
   ...
   //http "github.com/gorilla/http/client"
	"net/url"
	"fmt"
	"time"
   T "text/template"
...
func (r *ReqOpts) Request() *http.Request {
   ...
   if r.Body != "" { 
		...
		tem, err := T.New("body").Parse(r.Body)
		if err != nil { panic(err) }
   	tem.Execute(&body, time.Now())
      fmt.Print("Got body : "+body.String())
   }
/*
var buff bytes.Buffer
   ...
   tem, err := T.New("body").Parse(r.Body)
   if err != nil { panic(err) }
   tem.Execute(&buff, time.Now())
   //var body = buff.String()
	//req, _ := http.NewRequest(r.Method, r.URL, strings.NewReader(body))
*/
