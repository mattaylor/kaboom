...
/*
	...
	client := &Client{Transport: tr, Timeout: time.Duration(b.Timeout) * time.Second}
*/
	for req := range ch {
		size := int64(0)
		s := time.Now()
		//resp, err := client.Do(req)
      status, _, _, err := g.DefaultClient.Do(req.Method, req.URL.String(), req.Header, req.Body) 
      code := status.Code
      t := time.Now().Sub(s)
/*
		if err == nil {
